...
class MukellefPaneli(tk.Toplevel):
    ...
    def _onay_sor(self, baslik, mesaj):
        """Thread-safe onay dialogu. Ana thread'de messagebox gösterip
        sonucu bekler. True/False döner."""
        sonuc = [None]
        olay = threading.Event()

        def _goster():
            sonuc[0] = messagebox.askyesno(baslik, mesaj, parent=self)
            olay.set()

        self.after(0, _goster)
        olay.wait()
        return sonuc[0]
                    ...
                    try:
                        indirilen = luca_cekme.cek_muavin(
                            ...
                            onay_callback=lambda: self._onay_sor(
                                "Muavin İndirme Onayı",
                                "Luca muavin dökümü indirilecek.\n"
                                "Devam edilsin mi?"))
